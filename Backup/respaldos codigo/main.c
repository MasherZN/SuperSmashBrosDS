    ...
    // snprintf(segundos, sizeof(segundos), "Segundos: %d %d %d %d ", seconds,btn_anid,currentRoom,mbgsY2);
       ...
       btn_anim++;
        ...
        seconds+= 1;
         ...
         // Mover el fondo 1 píxel a la izquierda
    	  ...
    	  NF_ScrollBg(0,1, bgX,0);  
		   ...
		   NF_CreateTiledBg(0, 1, "blank");
           NF_CreateTiledBg(1, 1, "blank");
        ...
        if (keysDown() & KEY_UP) {
		 ...
		 if (keysDown() & KEY_DOWN && btn_anid==0) {
             mmEffectEx(&cursor); 
              NF_ClearTextLayer(1,0);
             btn_frame=0;
             btn_anid=2;  
        }     
         if (keysDown() & KEY_DOWN && btn_anid==1) {
             mmEffectEx(&cursor); 
              NF_ClearTextLayer(1,0);
             btn_frame=0;
             btn_anid=4;  
        }     
         if (keysDown() & KEY_UP && btn_anid==4) {
             mmEffectEx(&cursor); 
            NF_ClearTextLayer(1,0);
             btn_frame=0;
             btn_anid=1;  
        }     
         if (keysDown() & KEY_UP && btn_anid==3) {
             mmEffectEx(&cursor); 
            NF_ClearTextLayer(1,0);
             btn_frame=0;
             btn_anid=1;  
         ...
         if (keysDown() & KEY_UP && btn_anid==2) {
         	 NF_ClearTextLayer(1,0);
             mmEffectEx(&cursor); 
             btn_frame=0;
             btn_anid=0;
        }
         if (keysDown() & KEY_UP && btn_anid==5) {
         	 NF_ClearTextLayer(1,0);
             mmEffectEx(&cursor); 
             btn_frame=0;
             btn_anid=1;
        }
          if (keysDown() & KEY_RIGHT && btn_anid>=5) {
         	 NF_ClearTextLayer(1,0);
             mmEffectEx(&cursor); 
             btn_frame=0;
             btn_anid=1;
